...
import { remarkDescriptionList } from '.';

describe('remarkDescriptionList', () => {
	it('should transform `list` to `descriptionList`', () => {
		const transformer = unified().use(remarkParse).use(remarkDescriptionList).freeze();
		...
		const result = transformer.runSync(transformer.parse(markdown).children.at(0)!);
		const expected = {
			children: [
				{
					children: [
