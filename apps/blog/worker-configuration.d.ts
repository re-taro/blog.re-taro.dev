/* eslint-disable */
// Generated by Wrangler by running `wrangler types` (hash: 5572b6db06e808837477c739c782c833)
// Runtime types generated with workerd@1.20250617.0 2025-04-09 nodejs_compat
declare namespace Cloudflare {
	interface Env {
		CONTENTS: Fetcher /* contents */;
	}
}
...
interface BasicImageTransformations {
	 ...
	 *
	 * - Scale-down: Similar to contain, but the image is never enlarged. If the image is larger than given width or height,
	 *   it will be resized. Otherwise its original size will be kept.
	 * - Contain: Resizes to maximum size that fits within the given width and height. If only a single dimension is given
	 *   (e.g. only width), the image will be shrunk or enlarged to exactly match that dimension. Aspect ratio is always
	 *   preserved.
	 * - Cover: Resizes (shrinks or enlarges) to fill the entire area of width and height. If the image has an aspect ratio
	 *   different from the ratio of width and height, it will be cropped to fit.
	 * - Crop: The image will be shrunk and cropped to fit within the area specified by width and height. The image will not
	 *   be enlarged. For images smaller than the given dimensions it's the same as scale-down. For images larger than the
	 *   given dimensions, it's the same as cover. See also trim.
	 * - Pad: Resizes to the maximum size that fits within the given width and height, and then fills the remaining area
	 *   with a background color (white by default). Use of this mode is not recommended, as the same effect can be more
	 *   efficiently achieved with the contain mode and the CSS object-fit: contain property.
...
interface RequestInitCfPropertiesImage extends BasicImageTransformations {
	 ...
	 * - Border - this will automatically trim the surroundings of an image based on it's color. It consists of three
	 *   properties:
	 *
	 ...
	 * - Json: instead of generating an image, outputs information about the image, in JSON format. The JSON object will
	 *   contain image size (before and after resizing), source image’s MIME type, file size, etc.
	 ...
	 * enabled, all metadata may have been removed already and this option may have no effect.
	 *
